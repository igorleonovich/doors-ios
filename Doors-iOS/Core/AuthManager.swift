...
class AuthManager {
    ...
    func signUp(newUser: NewUserOutput, completion: @escaping (Swift.Error?) -> Void) {
        ...
        guard let url = URL(string: "\(ConstantsOld.baseURL)/auth/register") else {
            return
        }
...
//                        }
    ...
    func logIn(login: LoginOutput, _ completion: @escaping (Swift.Error?) -> Void) {
        ...
        guard let url = URL(string: "\(ConstantsOld.baseURL)/auth/login") else {
            return
        }
        var request = URLRequest(url: url)
        do {
    ...
    func refreshToken(_ completion: @escaping (Swift.Error?) -> Void) {
        ...
        guard let url = URL(string: "\(ConstantsOld.baseURL)/auth/accessToken") else {
            return
        }
        var request = URLRequest(url: url)
        do {
