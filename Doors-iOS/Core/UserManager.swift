...
class UserManager {
    ...
    func getUserProfile(_ completion: @escaping (Swift.Error?) -> Void) {
        ...
        guard let url = URL(string: "\(ConstantsOld.baseURL)/users/me") else {
            return
        }
    ...
    func activateDoorsService(doorsService: DoorsService, _ completion: @escaping (Swift.Error?) -> Void) {
        ...
        guard let url = URL(string: "\(ConstantsOld.baseURL)/doors-services/\(doorsService.rawValue)/activate") else {
            return
        }
        var request = URLRequest(url: url)
